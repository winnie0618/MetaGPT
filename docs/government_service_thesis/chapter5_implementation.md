...
`schema.py` 定义 `ServiceIntent`、`PolicyEvidence`、`MaterialItem`、`ProcessStep`、`RiskAssessment`、`TraceRecord` 和 `ServiceResponse` 等数据结构。`knowledge_base.py` 提供关键词检索、本地哈希 FAISS 检索、TF-IDF 统计向量检索和可选中文 embedding 语义检索，其中哈希 FAISS 索引持久化到 `workspace/government_service/rag`，语义 embedding 索引持久化到 `workspace/government_service/embedding_rag`。`trace_record.py` 将执行过程写入 `workspace/government_service/traces`，并通过 `TraceRecordStore` 支持按 `trace_id` 查询和最近记录读取。
...
打开 `http://127.0.0.1:8765` 后，可在页面中切换 `FAISS`、`Keyword`、`TF-IDF` 和 `Embedding` 四种检索后端，以及 `Template`、`RAG+LLM` 和 `LLM` 三种回答模式，并查看最终回答、政策依据、材料清单、办理步骤、风险等级和 `trace_id`。页面还支持按 `trace_id` 查询追溯链路，展示执行动作序列、检索文档、风险判断、知识库状态和回答模式。
